...
func logFood() -> Food? {
    guard let foodText = foodTextField.text,
          let caloriesString = caloriesTextField.text else {return nil}
    guard let calories =  Int(caloriesString) else {return nil}
...
if let fooding = loggedFood {
    print("The User just ate some \(fooding.name)")
}
